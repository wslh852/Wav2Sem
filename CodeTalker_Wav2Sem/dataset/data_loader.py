...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.read_audio:
            return torch.FloatTensor(audio),torch.FloatTensor(vertice), torch.FloatTensor(template), torch.FloatTensor(one_hot), file_name, text
        else:
    ...
    for r, ds, fs in os.walk(audio_path):
        for f in tqdm(fs):
            if f.endswith("wav"):
                ...
                sentencestext_path = '/home/lh/lihao/wav2bertcvpr2024/codetalker/CodeTalker-main/vocaset/sentencestext'
                text_path = os.path.join(sentencestext_path, subject_id)+'.txt'
                with open(text_path, 'r') as file:
                        specific_line = file.readlines()
                text = specific_line [int(f[-6:-4])-1][:-2]
                data[key]["text"] = text 
