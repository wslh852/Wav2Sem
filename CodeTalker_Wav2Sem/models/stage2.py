...
from models.wav2bert import wav2vecconfig, Wav2Bert,wav2veclargeconfig
...
class CodeTalker(BaseModel):
    def __init__(self, args):
        ...
        self.config = wav2vecconfig()
        self.Wav2Bert= Wav2Bert(self.config)
        checkpoint = torch.load('/home/lh/lihao/wav2bertcvpr2024/wav2bert/CodeTalker-main/save_data/model/model/model.pth.tar', map_location=lambda storage, loc: storage.cpu())
        load_state_dict(self.Wav2Bert, checkpoint['state_dict'])

        for param in self.Wav2Bert.parameters():
             param.requires_grad = False
        # wav2vec 2.0 weights initialization
        self.audio_encoder.feature_extractor._freeze_parameters()
        self.audio_feature_map = nn.Linear(768, args.feature_dim)
        self.bert_feature_map = nn.Linear(768, 768)
        # motion encoder
    ...
    def forward(self, audio, template, vertice, one_hot,text, criterion):
        ...
        hidden_states_bert = self.Wav2Bert(audio)
        hidden_states_bert = self.bert_feature_map(hidden_states_bert.mean(axis=1))

        # encoded_input = self.tokenizer(text, return_tensors='pt')
        # encoded_input = {k: v.to('cuda') for k, v in encoded_input.items()}
        # text_hidden_states = self.Bert(**encoded_input).last_hidden_state.mean(dim=1).unsqueeze(1)
        hidden_states = hidden_states +  hidden_states_bert
    ...
    def predict(self, audio, template, one_hot, text,one_hot2=None, weight_of_one_hot=None):
        ...
        #text_hidden_states = self.Bert(**encoded_input).last_hidden_state.mean(dim=1).unsqueeze(1)
        hidden_states_bert = self.Wav2Bert(audio)
        hidden_states_bert = hidden_states_bert.mean(axis=1)
        hidden_states = hidden_states + hidden_states_bert
        if self.dataset == "BIWI":
            frame_num = hidden_states.shape[1]//2
