...
cfg = get_parser('/home/lh/lihao/wav2bertcvpr2024/wav2bert/CodeTalker_Wav2Sem/config/vocaset/stage2.yaml')
...
def test(model, test_loader):
    ...
    with torch.no_grad():
        for i, (audio, vertice, template, one_hot_all, file_name,text) in enumerate(test_loader):
            ...
            if train_subject in train_subjects_list:
                ...
                for iter in range(one_hot_all.shape[-1]):
                    ...
                    one_hot = one_hot_all[:,iter,:]
                    prediction = model.predict(audio, template, one_hot, text)
                    prediction = prediction.squeeze()
                    np.save(os.path.join(save_folder, file_name[0].split(".")[0]+"_condition_"+condition_subject+".npy"), prediction.detach().cpu().numpy())
